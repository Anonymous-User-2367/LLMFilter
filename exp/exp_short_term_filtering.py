...
class Exp_Short_Term_Filtering(Exp_Basic):
    def __init__(self, args):
        super(Exp_Short_Term_Filtering, self).__init__(args)
    ...
    def vali(self, train_loader, vali_loader, criterion):
        ...
        with torch.no_grad():
            ...
            id_list = np.arange(0, B, self.args.window_length)
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            ...
            outputs = outputs[0, self.args.window_length:, :]
            preds = outputs.detach().cpu().numpy()
            trues = y[0, self.args.window_length:, :]
            
            x = x.detach().cpu().numpy()
            obs = x[0, self.args.window_length:, :]
        ...
        inference_time = (end_time - start_time) / (B - self.args.window_length) 
