...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Define configuration for LoRA (Low-Rank Adaptation)
        lora_config = LoraConfig(
            r=16,  # Rank of the low-rank matrix; controls the complexity of the adaptation
            lora_alpha=16,  # Scaling factor for the low-rank adapters, which determines their weight
            lora_dropout=0.0,  # Dropout probability for the low-rank adapters (0.0 means no dropout)
            target_modules=["q_proj", "v_proj"],  # Specifies which layers (modules) should apply LoRA (in this case, q_proj and v_proj)
        )
        ...
        # Load LLaMA model tokenizer with pre-trained weights
