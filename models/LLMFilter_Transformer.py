...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if configs.mlp_hidden_layers == 0:
            ...
            self.encoder = nn.Linear(self.enc_dim, self.hidden_dim_of_transformer)
            self.decoder = nn.Linear(self.hidden_dim_of_transformer, self.dec_dim)
        else:
            if not configs.use_multi_gpu or (configs.use_multi_gpu and configs.local_rank == 0):
                print("use mlp as tokenizer and detokenizer")
            self.encoder = MLP(self.enc_dim, self.hidden_dim_of_transformer, 
                            configs.mlp_hidden_dim, configs.mlp_hidden_layers, 
                            configs.dropout, configs.mlp_activation)
            self.decoder = MLP(self.hidden_dim_of_transformer, self.dec_dim,
                            configs.mlp_hidden_dim, configs.mlp_hidden_layers,
                            configs.dropout, configs.mlp_activation) 
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=self.hidden_dim_of_transformer, nhead=8, dim_feedforward=2048)
        self.transformer_encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=self.hidden_layers_of_transformer)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        outputs=self.transformer_encoder(times_embeds.permute(1, 0, 2)).permute(1, 0, 2)
