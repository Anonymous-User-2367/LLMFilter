...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if configs.llm_ckp_dir == 'model/opt-125m':
            self.hidden_dim_of_opt = 768
        elif configs.llm_ckp_dir == 'model/opt-350m':
            self.hidden_dim_of_opt = 1024
        elif configs.llm_ckp_dir == 'model/opt-1.3b':
            self.hidden_dim_of_opt = 2048
        elif configs.llm_ckp_dir == 'model/opt-2.7b':
            self.hidden_dim_of_opt = 2560
        elif configs.llm_ckp_dir == 'model/opt-6.7b':
            self.hidden_dim_of_opt = 4096
        else:
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # Encode tokens
        input_embeds = self.encoder(fold_out)  # Shape: [batch_size, token_num, hidden_dim_of_opt]
        ...
        outputs = self.opt(
            inputs_embeds=input_embeds)[0]  # Shape: [batch_size, prompt_token_len + token_num, hidden_dim_of_opt]
    ...
    def generate_prompt_embeddings(self, bs):
        ...
        Returns:
            prompt_embeddings (Tensor): Generated prompt embeddings. Shape: [batch_size, prompt_token_len, hidden_dim_of_opt].
        """
        ...
        prompt_embeddings = self.opt.get_input_embeddings()(prompt.to(self.device))  # Shape: [batch_size, prompt_token_len, hidden_dim_of_opt]
    ...
    def apply_prompt(self, input_embeds, batch_size):
        ...
        Args:
            input_embeds (Tensor): The input embeddings. Shape: [batch_size, token_num, hidden_dim_of_opt].
